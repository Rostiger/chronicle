...
function Databox () {
  ...
  let fullHeight = 0
	...
	this.start = function () {
		years = database.stats.years
		fullHeight = years * 12 * monthHeight
		this.update()
	}
    ...
    this.databox.innerHTML = `<h1>${year}-${m}</h1>`
    ...
    if (database.years[year]) {
	    ...
	    if (months) {
	    	for (const c in database.settings.CATEGORIES) {
	    		...
	    		this.databox.innerHTML += `<h2 style="color:${color}">${database.settings.CATEGORIES[c].NAME}</h2>`
					this.databox.innerHTML += category ? `<p>${category.description.toUpperCase()}</p>` : '<p>---</p>'
	    	}
	    }
	  }
	}
